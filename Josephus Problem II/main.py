...
if __name__ == '__main__':
        ...
        while killed < n - 1:
            ...
            while alive <= k:
                ...
                if A[remove_pos] == 1:
                    alive += 1
            alive = 1
            A[remove_pos] = 0  # kill the person at remove_pos
            res.append(remove_pos + 1)
            killed += 1
            remove_pos += 1
            remove_pos %= n

            while A[remove_pos] == 0:
                remove_pos += 1
                remove_pos %= n
        res.append(remove_pos + 1)
        return res
    ...
    def josephus(n: int, k: int):
        # SortedList O(LogN) remove/insert
        items = SortedList(list(range(1, n + 1)))
        start = 0
        res = []
        while n:
            pos = (start + k) % n
            res.append(items[pos])
